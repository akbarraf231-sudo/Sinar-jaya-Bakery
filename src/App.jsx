...
const Home = ({products,onCat,onProd,cart,onCart,heroBg,loading,onTrack}) => {
  ...
  return(<Shell>
    ...
    <div className="px-5 -mt-4 relative z-10"><div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-4 flex items-center gap-3 mb-4"><span className="text-lg">⚡</span><p className="text-sm text-stone-600 flex-1"><span className="font-semibold text-amber-800">Slot terbatas</span> setiap hari — pesan sekarang!</p></div>
      <div className="flex flex-col gap-3 max-w-xs mx-auto mb-3"><Btn onClick={()=>onCat("special")} full>🎉 Special Selection</Btn><Btn onClick={()=>onCat("classic")} full>🍩 Classic Selection</Btn></div>
      <button onClick={onTrack} className="w-full bg-white rounded-2xl shadow-sm border border-stone-100 p-4 flex items-center justify-center gap-2 hover:border-amber-200 hover:shadow-md transition-all"><span>📦</span><span className="text-sm font-semibold text-amber-800">Cek Status Pesanan</span><span className="text-stone-300">→</span></button>
    </div>
